...
def load_img_single_category(annos, imgid, marks):
    img = annos["imgs"][imgid]
    categories = []
    ...
    for mc in reversed(most_common):
        if mc[1] == 1 and mc[0] in marks:
            return mc[0]
    return None

def load_mask(annos, category, imgid, imgdata):
    img = annos["imgs"][imgid]
...
def draw_all(annos, datadir, imgid, imgdata, color=(0,1,0), have_mask=True, have_label=True):
    img = annos["imgs"][imgid]
    if have_mask:
        mask = load_mask(annos, datadir, imgid, imgdata)[0]
        imgdata = imgdata.copy()
        imgdata[:,:,0] = np.clip(imgdata[:,:,0] + mask*0.7, 0, 1)
    for obj in img['objects']:
        rbox = obj['ellipse']
        rbox = ((rbox[0][0], rbox[0][1]), (rbox[1][0], rbox[1][1]), rbox[2])
        cv2.ellipse(imgdata, rbox, 1, -1)
        ss = obj['category']
