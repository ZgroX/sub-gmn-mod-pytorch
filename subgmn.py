...
class sub_GMN(torch.nn.Module):
    def __init__(self, GCN_in_size, GCN_out_size, NTN_k, mask=False, is_softmax=True):
        ...
        self.mask = mask
        # layers
        self.GCN = three_gcn(in_size=self.GCN_in_size, out_size=self.GCN_out_size, is_softmax=is_softmax)
    ...
    def forward(self, x_d, edge_index_d, x_q, edge_index_q, batch_index_d, batch_index_q):  # b_same bx5x8
        ...
        att1 = att_layer(batch_q_em=q1, batch_da_em=da1, is_softmax=self.is_softmax)  # att bx1x5x18
        ...
        # 2
        att2 = att_layer(batch_q_em=q2, batch_da_em=da2, is_softmax=self.is_softmax)  # att bx1x5x18
        ...
        # 3
        att3 = att_layer(batch_q_em=q3, batch_da_em=da3, is_softmax=self.is_softmax)  # att bx1x5x18
        ...
        # end
        end = [torch.cat([e1, e2, e3, h1, h2, h3], dim=0) for e1, e2, e3, h1, h2, h3 in
